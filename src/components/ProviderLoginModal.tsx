...
export const ProviderLoginModal: React.FC<ProviderLoginModalProps> = ({
  ...
  const handleNonPremiumClick = (actionName = "E-Posta & Bulut Girişi") => {
    if (!isPremium && onOpenUpgradeModal) {
      handleClose();
      onOpenUpgradeModal(actionName);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isPremium) {
      handleNonPremiumClick(mode === "register" ? "Yeni Hesap Kaydı (PRO)" : "E-Posta ile Giriş (PRO)");
      return;
    }
      ...
      <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[1000] flex items-center justify-center p-4">
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {!isPremium ? (
                    <span className="px-2 py-0.5 bg-gradient-to-r from-amber-500/20 to-amber-500/30 border border-amber-500/40 text-amber-600 dark:text-amber-400 rounded-full text-[9px] font-black uppercase tracking-wider flex items-center gap-1 shadow-xs">
                      <Lock className="w-2.5 h-2.5" /> PRO KİLİTLİ
                    </span>
                  ) : (
                    <span className="px-2 py-0.5 bg-emerald-500/20 border border-emerald-500/40 text-emerald-600 dark:text-emerald-400 rounded-full text-[9px] font-black uppercase tracking-wider flex items-center gap-1 shadow-xs">
                      <span>👑</span> PRO AKTİF
                    </span>
                  )}
                </div>
          ...
          <div className="p-5 sm:p-6 space-y-4 max-h-[80vh] overflow-y-auto">
            ...
            {/* 2. PREMİUM GİRİŞ & KAYIT KİLİTLİ ALANI (EĞER PREMİUM DEĞİLSE) */}
            {!isPremium ? (
              <div className="p-5 bg-gradient-to-br from-amber-500/15 via-indigo-500/10 to-amber-500/20 border-2 border-amber-400/60 dark:border-amber-500/50 rounded-3xl text-left shadow-lg space-y-4 relative overflow-hidden">
                <div className="flex items-start gap-3.5">
                  <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 text-white flex items-center justify-center shrink-0 shadow-md shadow-amber-500/20">
                    <Lock className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="px-2 py-0.5 bg-amber-500 text-slate-950 text-[9px] font-black rounded-md uppercase tracking-wider">
                        👑 SADECE PREMİUM ÜYELER
                      </span>
                    </div>
                    <h3 className="text-sm font-black text-slate-900 dark:text-white mt-1">
                      E-Posta Girişi & Bulut Senkronizasyonu
                    </h3>
                    <p className="text-xs text-slate-600 dark:text-slate-300 font-medium mt-1 leading-relaxed">
                      E-posta adresiyle hesap oluşturma, şifre ile giriş yapma ve tüm cihazlarınız arasında anlık bulut eşitleme <strong>Bütçem PRO</strong> sürümüne özeldir.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-1 border-t border-amber-400/30">
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-200">
                    ...
                    <span>Çoklu cihaz senkronizasyonu ve otomatik yedekleme</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-200">
                    <Sparkles className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                    <span>Telefon değişse bile verilerinizi anında geri getirme</span>
                  </div>
                </div>

                <button
                  type="button"
                  onClick={() => handleNonPremiumClick("E-Posta & Bulut Girişi")}
              ...
              /* PREMİUM ÜYELER İÇİN AKTİF VE TAM AÇIK FORM */
              <div className="space-y-4">
                {/* Mode Switcher Tabs */}
                <div className="grid grid-cols-2 p-1 bg-slate-100 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 w-full">
                  <button
                    type="button"
                    onClick={() => {
                      setMode("login");
                ...
                <div className="p-3 bg-gradient-to-r from-indigo-500/10 via-purple-500/10 to-emerald-500/10 border border-indigo-500/20 rounded-2xl flex items-start gap-2.5 text-left">
                  ...
                  <p className="text-[11px] text-slate-700 dark:text-slate-300 leading-relaxed font-medium">
                    {mode === "register"
                      ? "Kayıt olduğunuzda tüm borç ve bütçe verileriniz Firebase Firestore bulutunda şifreli olarak yedeklenir."
                      : mode === "forgot"
                      ? "E-posta adresinizi girin; şifrenizi sıfırlamanız için size bir kurtarma bağlantısı gönderilecektir."
                      : "Giriş yaptığınızda diğer cihazlarınızdaki tüm bütçe kayıtlarınız anında eşitlenir."}
                  </p>
                </div>
                ...
                {isLoading && syncLogs.length > 0 ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-3.5 text-left">
                    {/* Security Notice */}
                    <div className="p-3 bg-amber-500/10 dark:bg-amber-950/30 border border-amber-500/30 rounded-2xl text-left space-y-1">
                      <div className="flex items-center gap-1.5 text-amber-600 dark:text-amber-400 font-black text-xs">
                        <span>⚠️</span>
                        <span>Şifre Belirleme:</span>
                      </div>
                      <p className="text-[11px] leading-relaxed text-amber-900/90 dark:text-amber-200/90 font-medium">
                        Kişisel Gmail şifrenizi değil, Bütçem Pro uygulamasına özel yeni bir şifre girin. İlk kez kullanıyorsanız "Kayıt Ol" sekmesini seçin.
                      </p>
                    </div>
